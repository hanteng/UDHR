...
for x in range(len(root)):
    udhrs_meta.append(root[x].attrib)

import pandas as pd
df = pd.DataFrame.from_dict(udhrs_meta)


dict_f2iso = df[['f','iso639-3']].set_index('f')['iso639-3'].to_dict()
dict_iso2f = df[['f','iso639-3']].set_index('iso639-3')['f'].to_dict()
